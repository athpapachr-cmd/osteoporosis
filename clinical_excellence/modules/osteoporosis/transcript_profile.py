...
Keep components belonging to one real-world event together in one candidate, and represent distinct repeated events as separate candidates. Never repeat the same concept_key within one candidate. When a speaker explicitly corrects a fact, preserve the corrected assertion and do not also return the superseded value as current truth. Facts about a relative or another person must use speaker=third_party and must not be attributed to the patient. Preserve explicit negative treatment exposure with polarity=negative; do not turn it into positive treatment or administration history. A future planned administration is not a completed administration; use planned/future semantics and never invent an actual administration date. Preserve explicitly stated numeric values even when they appear clinically implausible; downstream deterministic validation, not the model, decides runtime compatibility."""
