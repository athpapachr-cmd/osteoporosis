...
try:
    from weekly_check import build_weekly_summary, empty_weekly_result
except Exception as _weekly_check_import_exc:
    print(f"[WEEKLY_TRIAGE] weekly_check import failed: {_weekly_check_import_exc!r}; using local fallback.")

    def empty_weekly_result(days: int = 7, error: str = "") -> dict[str, Any]:
        return {
            "period_days": days,
            "total_calls": 0,
            "succeeded_count": 0,
            ...
            "failed_count": 0,
            "caller_abandoned_no_intent": 0,
            "success_rate_pct": None,
            "failed": [],
            "needs_review": [],
            "succeeded": [],
            "intent_completion_rate_pct": None,
            "by_intent": {},
            "latency": {},
            "scorecard": {},
            "quality_gate": {
                "status": "empty",
                "reason": "Δεν υπάρχουν διαθέσιμα δεδομένα για την περίοδο.",
            },
            "summary": {
                "total_calls": 0,
                "completed_calls": 0,
    ...
    def build_weekly_summary(
        ...
        *,
        ...
        audit_counters: dict[str, Any] | None = None,
    ) -> dict[str, Any]:
        audit_counters = audit_counters or {}
        total = int(audit_counters.get("total_calls") or len(recent or []))
        completed = int(audit_counters.get("completed_successfully") or completed_calls or 0)
        unresolved = int(audit_counters.get("unresolved_count") or max(total - completed, 0))
        return {
            "total_calls": total,
            "completed_calls": completed,
            "unresolved_calls": unresolved,
            "messages": int(audit_counters.get("message_success") or 0),
            "appointments": int(audit_counters.get("booking_success") or 0),
            "urgent_messages": int(audit_counters.get("urgent_count") or 0),
            "transfers": int(audit_counters.get("transfer_connected") or audit_counters.get("transfer_attempted") or 0),
            "stuck_or_silence": int(audit_counters.get("stuck_or_silence_count") or 0),
            "tool_errors": int(audit_counters.get("tool_errors") or 0),
        }
...
from appointment_utils import (
    apply_ordinary_callback_repeat_policy,
    ...
    message_guard_key,
    message_spoken_confirmation,
    MESSAGE_UPDATE_SPOKEN_CONFIRMATION,
    same_call_message_update_allowed,
...
def _state_flags_from_intent_result(result: dict[str, Any]) -> dict[str, Any]:
    ...
    flags: dict[str, Any] = {
        ...
        "callback_request_count": result.get("callback_request_count"),
        "pending_transfer_request": result.get("pending_transfer_request"),
        "transfer_authorized": result.get("transfer_authorized"),
    }
...
def _explicit_message_request_token(token: str) -> bool:
    """True only when the caller clearly abandons booking and asks for message/callback."""
    token = normalize_guard_text(token or "")
    explicit_phrases = (
        "θελω να αφησω μηνυμα",
        "να αφησω μηνυμα",
        "αφησω μηνυμα",
        "στειλε μηνυμα",
        "στειλτε μηνυμα",
        ...
        "να ενημερωσεις τον γιατρο",
        "να ενημερωσετε τον γιατρο",
        "να με παρει ο γιατρος",
        "να με παρετε τηλεφωνο",
        "να μου τηλεφωνησει",
        "να μου τηλεφωνησετε",
        "να μιλησω με τον γιατρο",
        "θελω να μιλησω με τον γιατρο",
        "δεν θελω ραντεβου",
        "οχι ραντεβου",
        "μονο μηνυμα",
    )
...
def _appointment_reason_context_from_text(raw_text: str) -> str:
    """Convert short appointment-reason answers into safe booking notes.

    During an active appointment flow, words like referral/mother/pain are
    context for the visit, not a new admin-message intent.
    """
    raw = (raw_text or "").strip()
    token = normalize_guard_text(raw)
    if not token:
        return ""
    if any(word in token for word in ("παραπεμπτικ", "παρα πεπτικ", "παραπεπτικ", "συσταση", "χαρτι", "γνωματευση")):
        return "Ιατρική επίσκεψη με παραπεμπτικό"
    if "μητερα" in token or "μαμα" in token:
        if any(word in token for word in ("πονο", "πονα", "κοκκαλ", "οστα", "μεση", "γονατο", "χερι", "ποδι", "ισχιο")):
            return raw
        return "Ιατρική επίσκεψη για τη μητέρα του καλούντος"
    if any(word in token for word in ("πονο", "πονα", "κοκκαλ", "οστα", "μεση", "γονατο", "ωμο", "χερι", "ποδι", "ισχιο", "πλατη", "αυχεν")):
        return raw
    if any(word in token for word in ("δεν ξερω", "εν ξερω", "δεν μπορω να πω", "θα ηθελα", "θα θελα")):
        return "Γενική ορθοπεδική επίσκεψη"
    # If the caller answers the reason question with a non-routing phrase, keep
    # the appointment alive and use a safe generic reason.
    return "Γενική ορθοπεδική επίσκεψη"
...
def _merge_sticky_route_state(
    ...
    *,
    ...
    # Appointment intent lock:
    # If we already asked for / entered a new appointment flow, do not let a
    # follow-up answer such as "παραπεμπτικό", "η μητέρα μου πονάει" or a vague
    # reason answer flip the flow to message/check existing appointment. Only a
    # clear "send message/call me" phrase is allowed to abandon the booking flow.
    if (
        previous_intent in {"appointment", "change"}
        and active_appointment_stage
        and str(merged.get("intent") or "") in {"message", "other"}
        and not _explicit_message_request_token(routing_token)
    ):
        reason_note = _appointment_reason_context_from_text(_routing_message_from_body(body))
        if active_appointment_stage_name == "collecting_reason" or reason_note:
            merged["intent"] = previous_intent
            merged["confidence"] = max(float(merged.get("confidence") or 0.0), 0.98)
            merged["is_confident"] = True
            merged["source"] = "sticky_appointment_reason_context_guard"
            merged["message_category"] = ""
            merged["appointment_triage_needed"] = False
            merged["appointment_triage_done"] = True
            merged["message_clean"] = reason_note or "Γενική ορθοπεδική επίσκεψη"
...
def _message_spoken_response(
    ...
    *parts: str,
    ...
    return message_spoken_confirmation(
        message_category,
        priority_level=priority_level,
        caller_wants_callback=_caller_wants_callback_from_text(*parts),
    )
...
def _message_state(phone_number: Optional[str], call_id: Optional[str]) -> dict[str, Any]:
    if call_id:
        ...
        if call_state:
            return call_state
        return {
            "caller_number": phone_number or "",
            "phone_number": phone_number or "",
            "active_call_id": call_id,
        }
    # Legacy callers without call_id may read phone state, but they are never
    # eligible for message updates because same-call identity cannot be proven.
    return get_conversation_state(phone_number=phone_number)
...
def _should_update_existing_message(
    ...
    if not same_call_message_update_allowed(previous_state, call_id):
        return False
...
@app.get(
    "/actions/patient_lookup",
    response_model=PatientLookupResponse,
    dependencies=[Depends(require_action_token)],
)
async def patient_lookup(phone: str = Query(..., min_length=3)):
    identity = _patient_identity_from_cache_by_phone(phone)
    ...
    return PatientLookupResponse(
        found=bool(display_name),
        display_name=display_name,
        first_name=first_name,
        last_name=last_name,
        source=identity.get("patient_lookup_source", ""),
    )
...
async def pre_route_intent_action(body: PreRouteIntentRequest):
    ...
    if previous_state.get("conversation_done"):
        try:
            state_age = time.monotonic() - float(
                previous_state.get("updated_at_monotonic") or 0
            )
        except (TypeError, ValueError):
            state_age = float("inf")
        recent_callback_continuation = (
            str(previous_state.get("current_message_category") or "")
            == "callback_request"
            and int(previous_state.get("callback_request_count") or 0) > 0
            and state_age <= 120
        )
        if recent_callback_continuation:
            previous_state["conversation_done"] = False
            print(
                "PRE_ROUTE_INTENT: continuing recent callback request "
                f"count={previous_state.get('callback_request_count')}"
            )
        else:
            clear_conversation_state(
                phone_number=caller_number,
    ...
    pending_transfer = previous_state.get("pending_transfer_request") or previous_state.get("current_stage") == "qualifying_transfer_origin"
    _HEALTHCARE_ORIGIN_MARKERS = {
        "ιατρικο κεντρο", "ιατρικο", "νοσοκομειο", "ακτινολογικο",
        "διαγνωστικο", "φαρμακειο", "κλινικη", "γιατρος", "ιατρος",
        "νοσηλευτης", "νοσηλευτρια", "φυσιοθεραπευτης", "φυσιοθεραπευτρια",
        "γραμματεια γιατρου", "πρωτες βοηθειες", "ιατρειο",
    }
    if pending_transfer:
        has_healthcare_origin = any(m in confirmation_token for m in _HEALTHCARE_ORIGIN_MARKERS)
        if has_healthcare_origin:
            result = {
                "intent": "message",
                "confidence": 1.0,
                "is_confident": True,
                "source": "transfer_origin_confirmed",
                "message_clean": routing_message,
                "message_category": "health_professional_callback",
                "priority_level": classify_priority_level(routing_message, "message"),
                "current_stage": "ready_to_transfer",
                "next_action": "transfer_to_doctor",
                "action_tool": "transfer_to_doctor",
                "transfer_authorized": True,
                "spoken_response": "Σας συνδέω με τον γιατρό, παρακαλώ περιμένετε.",
            }
            previous_state["pending_transfer_request"] = False
            update_conversation_state(
                phone_number=caller_number,
                call_id=body.call_id,
            ...
            return PreRouteIntentResponse(**result)
        else:
            already_sent = bool(previous_state.get("message_sent"))
            result = {
                "intent": "message",
                "confidence": 1.0,
                "is_confident": True,
                "source": "transfer_origin_declined",
                "message_clean": str(
                    previous_state.get("current_message") or routing_message
                ),
                "message_category": "callback_request",
                "priority_level": "C",
                "current_stage": "message_sent" if already_sent else "ready_to_message",
                "next_action": "end_or_new_request" if already_sent else "send_message",
                "spoken_response": (
                    "Έχω ήδη ενημερώσει τον γιατρό να σας καλέσει μόλις μπορέσει."
                    if already_sent
                    else "Θα ενημερώσω τον γιατρό να σας καλέσει μόλις μπορέσει."
                ),
            }
            previous_state["pending_transfer_request"] = False
            update_conversation_state(
                ...
                call_id=body.call_id,
                pending_transfer_request=False,
                caller_wants_callback=True,
            )
            result = _merge_sticky_route_state(result, state=previous_state, body=body)
            return PreRouteIntentResponse(**result)

    result = apply_ordinary_callback_repeat_policy(result, previous_state)
...
async def _recover_start_only_message_from_webhook(
    ...
    *,
    ...
    update_conversation_state(
        ...
        active_call_id=payload.call_id or "",
...
async def replace_appointment_attendee_action(body: ReplaceAppointmentAttendeeRequest):
    """Replace the visible patient/attendee for an existing appointment.

    Μετά το κλείσιμο του Cal.com v1 API (8 Απριλίου 2026), χρησιμοποιεί
    αποκλειστικά v2 endpoints:
      - PATCH /v2/bookings/{uid}/attendees/{attendeeId}  (αλλαγή ονόματος)
      - GET /v2/bookings/{uid}                            (εύρεση booking)
      - GET /v2/bookings/{uid}/attendees                  (εύρεση attendee ID)
    """
    ...
    if not booking_uid:
        return ReplaceAppointmentAttendeeResponse(
            success=False,
            message="Χρειάζεται booking_uid ή cal_uid.",
        )
    if not full_name:
        return ReplaceAppointmentAttendeeResponse(
            success=False,
            booking_uid=booking_uid,
            message="Χρειάζεται τουλάχιστον όνομα ή επώνυμο.",
        )
    ...
    setmore_appointment_key = body.setmore_appointment_key
    booking_id_str = str(body.booking_id or "")
    attendee_id_str = str(body.attendee_id or "")

    # ── Βήμα 1: Φέρνουμε booking + attendees μέσω v2 ──
    try:
        booking_resp = await cal.get_booking_by_uid_v2(booking_uid)
        raw["booking"] = booking_resp
        booking_data = _booking_data_from_response(booking_resp)
        if not booking_id_str:
            booking_id_str = str(booking_data.get("id") or "")
    except Exception as exc:
        errors.append(f"cal_get_booking_failed: {exc}")
        booking_data = {}

    try:
        attendees_resp = await cal.get_booking_attendees_v2(booking_uid)
        raw["attendees"] = attendees_resp
        attendee_list = _attendee_list_from_payload(attendees_resp)
        if attendee_list and not attendee_id_str:
            attendee_id_str = str(attendee_list[0].get("id") or "")
    except Exception as exc:
        errors.append(f"cal_get_attendees_failed: {exc}")
        attendee_list = []

    # ── Βήμα 2: PATCH attendee μέσω v2 (αντί v1) ──
    if attendee_id_str and booking_uid:
        phone_number = (
            body.phone_number
            or body.caller_number
            or ""
        ).strip()
        new_email = body.email or _sms_email_from_phone_or_name(
            phone_number, full_name, cal
        )

        try:
            patch_result = await cal.update_attendee_v2(
                booking_uid=booking_uid,
                attendee_id=attendee_id_str,
                name=full_name,
                email=new_email,
                phone_number=phone_number or None,
                timezone_str=body.timezone or "Asia/Nicosia",
                language=body.language or "el",
            )
            raw["cal_v2_patch_attendee"] = patch_result
            cal_updated = True
            print(f"REPLACE ATTENDEE v2 PATCH OK: uid={booking_uid} attendee={attendee_id_str} name={full_name}")
        except Exception as exc:
            errors.append(f"cal_v2_patch_attendee_failed: {exc}")
            print(f"REPLACE ATTENDEE v2 PATCH FAILED: {exc}")
    else:
        errors.append("missing_attendee_id_or_booking_uid")

    # ── Βήμα 3: Setmore update (keep existing logic) ──
    if not setmore_appointment_key:
        try:
            setmore_appointment_key_found = await _find_setmore_key_for_booking(
                cal_uid=booking_uid,
                start_iso=body.start_iso,
            )
            if setmore_appointment_key_found:
                setmore_appointment_key = setmore_appointment_key_found
        except Exception as exc:
            errors.append(f"setmore_find_key_failed: {exc}")

    if setmore_appointment_key:
        try:
            setmore_result = await update_setmore_appointment_customer(
                appointment_key=setmore_appointment_key,
                customer_name=full_name,
                customer_phone=(body.phone_number or body.caller_number or "").strip(),
                customer_email=body.email,
            )
            raw["setmore_update"] = setmore_result
            setmore_updated = bool(
                isinstance(setmore_result, dict) and setmore_result.get("success")
            )
        except Exception as exc:
            errors.append(f"setmore_update_customer_failed: {exc}")
            raw["setmore_update"] = {"success": False, "error": str(exc)}

    # ── Βήμα 4: Conversation state ──
    phone = _caller_number_from_body(body) or (body.phone_number or "").strip()
    if phone:
        update_conversation_state(
            phone_number=phone,
            call_id=body.call_id,
    ...
    success = cal_updated
    message = "Το ραντεβού ενημερώθηκε." if cal_updated else "Δεν μπόρεσε να ενημερωθεί το Cal."
    ...
    return ReplaceAppointmentAttendeeResponse(
        ...
        setmore_appointment_key=setmore_appointment_key,
        message=message,
        errors=errors,
        raw=raw,
        state=_message_state(phone, body.call_id) if phone else None,
    )
...
async def message_action(request: Request):
    ...
    try:
        ...
        priority_level = resolve_message_priority(
            f"{payload.message_clean or ''} {payload.message or ''}",
            payload.reason_for_call,
            requested_priority=payload.priority_level,
        ...
        if _should_update_existing_message(
            ...
            return MessageResponse(
                ...
                spoken_response=MESSAGE_UPDATE_SPOKEN_CONFIRMATION,
                state=state,
            )
        ...
        if not claim_action_once(dedupe_key, ttl_seconds=15 * 60):
            ...
            duplicate_spoken_response = _message_spoken_response(
                message_category,
                incoming_message,
                priority_level=priority_level,
            )
            return MessageResponse(
                ...
                spoken_response=duplicate_spoken_response,
                state=state,
            )
...
async def livekit_call_complete(request: Request):
    ...
    state = {
        ...
        "priority_alert_sent": bool(data.get("priority_alert_sent")),
        "urgent_outbound_sent": bool(data.get("urgent_outbound_sent")),
        "message_context_updated": bool(data.get("message_context_updated")),
        "transfer_attempted": bool(data.get("transfer_attempted")),
        "transfer_status": data.get("transfer_status") or "",
        "transfer_method": data.get("transfer_method") or "",
        "information_completed": bool(data.get("information_completed")),
    ...
    _terminal_type = (
        _outcome_evaluation["actual_outcome"]
        if _outcome_evaluation["actual_outcome"] != "none"
        else (data.get("outcome") or "call")
    )
    _call_event = {
        "type": _terminal_type,
        "call_id": call_id,
        ...
        "incomplete": not bool(_outcome_evaluation["intent_completed"]),
        "conversation_done": bool(data.get("conversation_done")),
        "message_sent": bool(data.get("message_sent")),
        "message_context_updated": bool(data.get("message_context_updated")),
        "booking_completed": bool(data.get("booking_completed")),
        ...
        "intended_outcome": _outcome_evaluation["intended_outcome"],
        "actual_outcome": _outcome_evaluation["actual_outcome"],
        "outcome_class": _outcome_evaluation["outcome_class"],
        "intent_completed": _outcome_evaluation["intent_completed"],
        "failure_point": _outcome_evaluation["failure_point"],
        "flow_quality": _outcome_evaluation["flow_quality"],
        ...
        "e2e_latencies_ms": data.get("e2e_latencies_ms") or [],
        "llm_ttft_ms": data.get("llm_ttft_ms") or [],
        "tts_ttfb_ms": data.get("tts_ttfb_ms") or [],
        "eou_delay_ms": data.get("eou_delay_ms") or [],
        "transcription_delay_ms": data.get("transcription_delay_ms") or [],
        "turn_callback_delay_ms": data.get("turn_callback_delay_ms") or [],
        "tool_latency_samples": data.get("tool_latency_samples") or [],
    }
    ...
    try:
        ...
        audit_row = record_call_audit(
            state=state,
            payload=payload,
            raw_data={**data, "source": "livekit", "nodes_visited": nodes_visited},
            raw_text="",
            phone=phone,
            call_id=call_id,
            disposition=disposition,
...
def _dashboard_audit_parse_date(row: dict[str, Any]) -> Optional[date]:
    """Parse a call-audit row date using Cyprus local date when available."""
    try:
        value = str(row.get("date_local") or "").strip()
        if value:
            return date.fromisoformat(value[:10])
    except Exception:
        pass
    for key in ("logged_at_local", "logged_at", "created_at"):
        ...
        try:
            ...
            if parsed.tzinfo is None:
                parsed = parsed.replace(tzinfo=TZ_NICOSIA)
            return parsed.astimezone(TZ_NICOSIA).date()
        except Exception:
            continue
    return None


def _dashboard_audit_pct(numerator: int | float, denominator: int | float) -> Optional[float]:
    try:
        denominator = float(denominator)
        if denominator <= 0:
            return None
        return round(100.0 * float(numerator) / denominator, 1)
    except Exception:
        return None


def _dashboard_audit_percentile(values: list[int], percentile: int) -> Optional[int]:
    ...
    return round(clean[lower] + (clean[upper] - clean[lower]) * fraction)


def _dashboard_audit_node_text(row: dict[str, Any]) -> str:
    parts: list[str] = []
    for key in ("outcome", "failure_point", "next_action", "current_stage", "summary", "message"):
        value = row.get(key)
        if value:
            parts.append(str(value))
    nodes = row.get("nodes_visited") or []
    if isinstance(nodes, list):
        parts.extend(str(node) for node in nodes)
    return normalize_guard_text(" ".join(parts))


def _dashboard_audit_bucket(rows: list[dict[str, Any]], *, days_divisor: int = 1) -> dict[str, Any]:
    """Aggregate audit rows defensively. Intent labels are agent/dashboard labels, not human accuracy."""
    from call_audit import (
        audit_result_override,
        classify_audit_attribution,
    ...
    transfer_connected = 0
    transfer_failed = 0
    urgent_count = 0
    tool_errors = 0
    stuck_or_silence = 0
    callback_needed = 0
    caller_abandoned_before_intent = 0
    caller_left_after_success = 0
    caller_left_unresolved = 0
    caller_left_after_intent = 0
    caller_left_before_agent_opportunity = 0
    caller_hangup_total = 0
    agent_failure_count = 0
    agent_success_count = 0
    manual_review_count = 0
    tool_error_manual_review = 0
    caller_left_after_message = 0
    caller_left_during_appointment = 0
    caller_left_after_message_success = 0
    unresolved_review_count = 0
    excluded_from_completion_denominator = 0
    failure_stage_distribution: dict[str, int] = {}
    reasoning_values: list[int] = []
    intent_distribution: dict[str, int] = {}

    for row in rows:
        intent = str(row.get("intent") or "unknown").strip().lower() or "unknown"
        intent_distribution[intent] = intent_distribution.get(intent, 0) + 1

        outcome = str(row.get("outcome") or "").strip().lower()
        outcome_class = str(row.get("outcome_class") or "").strip().lower()
        failure_point = str(row.get("failure_point") or "").strip().lower()
        next_action = str(row.get("next_action") or "").strip().lower()
        node_text = _dashboard_audit_node_text(row)

        is_completed = bool(row.get("success") or row.get("intent_completed"))
        caller_abandoned = failure_point == "caller_abandoned_no_intent"
        stalled = bool(row.get("stalled"))
        result_class = str(row.get("result_class") or "").strip().lower()
        if not result_class:
            result_class = classify_audit_result(
                success=is_completed,
                failure_point=failure_point,
                disposition=str(row.get("disposition") or ""),
                stalled=stalled,
            )
        reviewed_override = audit_result_override(str(row.get("call_id") or ""))
        if reviewed_override:
            result_class = reviewed_override
        attribution = classify_audit_attribution(row, result_class=result_class)
        row.update(attribution)
        agent_success_count += int(
            attribution["audit_outcome"] == "agent_success"
        )
        manual_review_count += int(bool(attribution["needs_manual_review"]))
        tool_error_manual_review += int(
            attribution["audit_outcome"] == "tool_error_needs_manual_review"
        )
        caller_left_after_message += int(
            attribution["audit_outcome"]
            == "caller_left_after_actionable_message_needs_review"
        )
        caller_left_during_appointment += int(
            attribution["audit_outcome"] == "caller_left_after_partial_appointment"
        )
        caller_left_after_message_success += int(
            attribution["audit_outcome"] == "agent_success"
            and intent in {"message", "urgent"}
            and bool(row.get("caller_left_after_terminal_success"))
        )
        unresolved_review_count += int(
            attribution["audit_outcome"] == "agent_failed"
            or bool(attribution["needs_manual_review"])
        )
        excluded_from_completion_denominator += int(
            bool(attribution["excluded_from_completion_denominator"])
        )
        caller_abandoned_before_intent += int(
            result_class == "caller_abandoned_before_intent"
        )
        caller_left_after_success += int(result_class == "caller_left_after_success")
        caller_left_unresolved += int(result_class == "caller_left_unresolved")
        caller_left_after_intent += int(result_class == "caller_left_after_intent")
        caller_left_before_agent_opportunity += int(
            result_class == "caller_left_before_agent_opportunity"
        )
        caller_hangup_total += int(
            result_class
            in {
                "caller_abandoned_before_intent",
                "caller_left_after_success",
                "caller_left_unresolved",
                "caller_left_after_intent",
                "caller_left_before_agent_opportunity",
            }
        )
        agent_failure = attribution["audit_outcome"] == "agent_failed"
        agent_failure_count += int(agent_failure)
        if agent_failure:
            failed_stage = str(row.get("failure_stage") or "").strip().lower()
            if not failed_stage:
            ...
            failure_stage_distribution[failed_stage] = (
                failure_stage_distribution.get(failed_stage, 0) + 1
            )
        completed += int(is_completed)
        message_success += int(
            outcome in {"message", "urgent"}
            or outcome_class
            in {
                "completed_message",
                "completed_message_update",
                "completed_urgent_message",
                "completed_urgent_action",
            }
            or (intent in {"message", "urgent"} and is_completed)
        )
        booking_success += int(outcome == "booking")
        cancellation_success += int(outcome in {"cancel", "cancellation"})
        urgent_count += int(
            intent == "urgent"
            or outcome == "urgent"
            or str(row.get("priority_level") or "").upper() == "A"
        )

        transfer_status = str(row.get("transfer_status") or "").strip().lower()
        transfer_seen = bool(
            row.get("transfer_attempted")
            or outcome == "transfer"
            or outcome_class.startswith("transfer_")
            or "transfer" in node_text
            or "warm_conference" in node_text
        )
        transfer_is_connected = (
            outcome_class == "transfer_connected"
            or transfer_status in {"connected", "completed", "answered"}
            or "doctor connected" in node_text
        )
        transfer_attempted += int(transfer_seen)
        transfer_connected += int(transfer_seen and transfer_is_connected)
        transfer_failed += int(
            transfer_seen
            and transfer_status in {"failed", "error", "no_answer"}
            and not is_completed
        )

        tool_errors += int(
            attribution["audit_outcome"] == "tool_error_needs_manual_review"
            or "error" in failure_point
            or "failed" in failure_point
            or "tool" in failure_point
            or "exception" in node_text
        )
        stuck_or_silence += int(
            not is_completed
            and agent_failure
            and (
                bool(stalled)
                or failure_point in {
                    "start_only",
                    "start_node",
                    "stuck_collecting_reason",
                    "stuck_collecting_clinic",
                    "stuck_collecting_name",
                    "message_not_sent",
                    "suggestions_not_called_or_no_slot",
                }
                or "silence" in node_text
            )
        )
        callback_needed += int(
            not is_completed
            and (
                failure_point == "message_not_sent"
                or next_action in {"send_message", "send_message_then_urgent_outbound"}
                or intent in {"message", "urgent"}
            )
        )
        try:
            value = row.get("reasoning_total_ms")
            if value not in (None, ""):
                reasoning_values.append(int(float(value)))
        except Exception:
            pass

    days_divisor = max(1, int(days_divisor or 1))
    excluded_caller_closed_before_completion = excluded_from_completion_denominator
    completion_denominator = total
    agent_opportunity_denominator = max(
        total - excluded_from_completion_denominator,
        0,
    )
    unresolved = unresolved_review_count
    caller_ended_incomplete = max(total - completed, 0)
    agent_controlled_denominator = agent_success_count + agent_failure_count

    def avg_count(value: int) -> float:
        return round(float(value) / days_divisor, 1)

    return {
        "total_calls": total,
        "total_calls_avg_per_day": avg_count(total),
        "completed_successfully": completed,
        "completed_successfully_avg_per_day": avg_count(completed),
        "completion_rate_pct": _dashboard_audit_pct(completed, completion_denominator),
        "task_completion_rate_pct": _dashboard_audit_pct(completed, total),
        "completion_denominator": completion_denominator,
        "agent_opportunity_denominator": agent_opportunity_denominator,
        "excluded_caller_closed_before_completion": excluded_caller_closed_before_completion,
        "caller_abandoned_before_intent": caller_abandoned_before_intent,
        "caller_abandoned_no_intent": caller_abandoned_before_intent,
        "caller_left_after_success": caller_left_after_success,
        "caller_left_unresolved": caller_left_unresolved,
        "caller_left_after_intent": caller_left_after_intent,
        "caller_left_before_agent_opportunity": caller_left_before_agent_opportunity,
        "caller_hangup_total": caller_hangup_total,
        "agent_failure_count": agent_failure_count,
        "agent_failure_count_avg_per_day": avg_count(agent_failure_count),
        "agent_success_count": agent_success_count,
        "agent_success_count_avg_per_day": avg_count(agent_success_count),
        "agent_controlled_denominator": agent_controlled_denominator,
        "agent_controlled_success_rate_pct": _dashboard_audit_pct(
            agent_success_count,
            agent_controlled_denominator,
        ),
        "manual_review_count": manual_review_count,
        "manual_review_count_avg_per_day": avg_count(manual_review_count),
        "tool_error_manual_review": tool_error_manual_review,
        "tool_error_manual_review_avg_per_day": avg_count(
            tool_error_manual_review
        ),
        "caller_left_after_message": caller_left_after_message,
        "caller_left_after_actionable_message_needs_review": (
            caller_left_after_message
        ),
        "caller_left_after_message_avg_per_day": avg_count(
            caller_left_after_message
        ),
        "caller_left_during_appointment": caller_left_during_appointment,
        "caller_left_after_partial_appointment": caller_left_during_appointment,
        "caller_left_during_appointment_avg_per_day": avg_count(
            caller_left_during_appointment
        ),
        "caller_left_after_message_success": caller_left_after_message_success,
        "caller_left_after_message_success_avg_per_day": avg_count(
            caller_left_after_message_success
        ),
        "failure_stage_distribution": dict(
            sorted(
                failure_stage_distribution.items(),
                key=lambda item: (-item[1], item[0]),
            )
        ),
        "message_success": message_success,
        "message_success_avg_per_day": avg_count(message_success),
        "booking_success": booking_success,
        "booking_success_avg_per_day": avg_count(booking_success),
        "cancellation_success": cancellation_success,
        "cancellation_success_avg_per_day": avg_count(cancellation_success),
        "transfer_attempted": transfer_attempted,
        "transfer_attempted_avg_per_day": avg_count(transfer_attempted),
        "transfer_connected": transfer_connected,
        "transfer_connected_avg_per_day": avg_count(transfer_connected),
        "transfer_failed": transfer_failed,
        "transfer_failed_avg_per_day": avg_count(transfer_failed),
        "urgent_count": urgent_count,
        "urgent_count_avg_per_day": avg_count(urgent_count),
        "tool_errors": tool_errors,
        "tool_errors_avg_per_day": avg_count(tool_errors),
        "stuck_or_silence_count": stuck_or_silence,
        "stuck_or_silence_count_avg_per_day": avg_count(stuck_or_silence),
        "unresolved_count": unresolved,
        "unresolved_count_avg_per_day": avg_count(unresolved),
        "caller_ended_incomplete_count": caller_ended_incomplete,
        "caller_ended_incomplete_count_avg_per_day": avg_count(
            caller_ended_incomplete
        ),
        "callback_needed": callback_needed,
        "callback_needed_avg_per_day": avg_count(callback_needed),
        "avg_latency_ms": round(sum(reasoning_values) / len(reasoning_values)) if reasoning_values else None,
        "p95_latency_ms": _dashboard_audit_percentile(reasoning_values, 95),
        "intent_distribution": dict(sorted(intent_distribution.items())),
    }


@app.get("/dashboard/audit-comparison")
async def dashboard_audit_comparison(key: str = ""):
    """Today's agent-performance audit vs yesterday, 7-day average and previous week.

    This intentionally does NOT claim intent accuracy unless human/golden labels
    exist. Intent values here are the labels produced by the agent/dashboard audit.
    """
    dashboard_key = os.getenv("DASHBOARD_KEY", "")
    if dashboard_key and key != dashboard_key:
        return JSONResponse({"error": "unauthorized"}, status_code=401)
    ...
    try:
        ...
        last_7_dates = {today - timedelta(days=i) for i in range(1, 8)}
        previous_week_dates = {today - timedelta(days=i) for i in range(8, 15)}

        rows = await asyncio.to_thread(load_call_audits, 21)
        rows_by_date: dict[date, list[dict[str, Any]]] = {}
        for row in rows:
            day = _dashboard_audit_parse_date(row)
            if day is None:
                continue
            rows_by_date.setdefault(day, []).append(row)

        today_rows = rows_by_date.get(today, [])
        yesterday_rows = rows_by_date.get(yesterday, [])
        last_7_rows = [row for day in last_7_dates for row in rows_by_date.get(day, [])]
        previous_week_rows = [row for day in previous_week_dates for row in rows_by_date.get(day, [])]

        result = {
            "today": _dashboard_audit_bucket(today_rows, days_divisor=1),
            "yesterday": _dashboard_audit_bucket(yesterday_rows, days_divisor=1),
            "last_7_days_avg": _dashboard_audit_bucket(last_7_rows, days_divisor=7),
            "previous_week_avg": _dashboard_audit_bucket(previous_week_rows, days_divisor=7),
            "intent_distribution_today": _dashboard_audit_bucket(today_rows)["intent_distribution"],
            "intent_distribution_last_7_days": _dashboard_audit_bucket(last_7_rows)["intent_distribution"],
            "generated_at": now_local.isoformat(timespec="seconds"),
            "timezone": "Asia/Nicosia",
            "sample_sizes": {
                "today": len(today_rows),
                "yesterday": len(yesterday_rows),
                "last_7_days": len(last_7_rows),
                "previous_week": len(previous_week_rows),
            },
            "note": "Intent labels are agent/dashboard labels, not human/golden accuracy.",
            "title": "Σημερινό audit απόδοσης agent",
        }
        return result
    except Exception as exc:
        print(f"[AUDIT_COMPARISON] failed: {exc}")
        return JSONResponse(
            {"error": "audit comparison unavailable", "detail": str(exc)},
            status_code=500,
        )
...
def _safe_empty_weekly_result(days: int = 7, error: str = "") -> dict[str, Any]:
    """Return a valid JSON-serializable weekly-triage object even on errors."""
    fallback = {
        "period_days": days,
        "total_calls": 0,
        "succeeded_count": 0,
        ...
        "quality_gate": {
            ...
            "reason": "Δεν υπάρχουν διαθέσιμα δεδομένα για την περίοδο."
            if not error
            else "Ο εβδομαδιαίος έλεγχος επέστρεψε ασφαλές κενό αποτέλεσμα μετά από σφάλμα.",
        },
        "summary": {
            "total_calls": 0,
            "completed_calls": 0,
    ...
    try:
        result = empty_weekly_result(days, error=error)
        if isinstance(result, dict):
            ...
            if not isinstance(result.get("summary"), dict):
                ...
                for key, value in fallback["summary"].items():
                    result["summary"].setdefault(key, value)
            return result
    except Exception as exc:
        print(f"[WEEKLY_TRIAGE] empty result helper failed: {exc!r}")
    return fallback


def _safe_weekly_summary(
    recent: list[dict[str, Any]],
    *,
    completed_calls: int,
    audit_counters: dict[str, Any],
) -> dict[str, Any]:
    """Always return the dashboard weekly-summary shape.

    The weekly panel must never fail just because the optional weekly_check
    helper raises or returns an unexpected object.
    """
    fallback = {
        "total_calls": int(audit_counters.get("total_calls") or len(recent or [])),
        "completed_calls": int(
            audit_counters.get("completed_successfully") or completed_calls or 0
        ),
        "unresolved_calls": int(
            audit_counters.get("unresolved_count")
            or max(
                int(audit_counters.get("total_calls") or len(recent or []))
                - int(audit_counters.get("completed_successfully") or completed_calls or 0),
                0,
            )
        ),
        "messages": int(audit_counters.get("message_success") or 0),
        ...
        "urgent_messages": int(audit_counters.get("urgent_count") or 0),
        "transfers": int(
            audit_counters.get("transfer_connected")
            or audit_counters.get("transfer_attempted")
            or 0
        ),
        "stuck_or_silence": int(audit_counters.get("stuck_or_silence_count") or 0),
        "tool_errors": int(audit_counters.get("tool_errors") or 0),
    ...
    try:
        ...
        if not isinstance(summary, dict):
            raise TypeError(f"weekly summary returned {type(summary).__name__}")
        for key, value in fallback.items():
            summary.setdefault(key, value)
        return summary
    except Exception as exc:
        print(f"[WEEKLY_TRIAGE] summary build failed: {exc!r}")
        return fallback


async def _dashboard_weekly_triage_impl(key: str = "", days: int = 7):
    """
    ...
    if not recent:
        return _safe_empty_weekly_result(days)
    ...
    failed, needs_review, succeeded, caller_abandoned = [], [], [], []
    intent_stats: dict[str, dict[str, int]] = {}

    # Doctor-reviewed caller_abandoned overrides (point 9 migration): a call the
    # doctor marked as caller_abandoned must leave manual review and appear in the
    # caller-abandoned bucket even if auto-detection did not catch it.
    _abandoned_call_ids: set[str] = set()
    try:
        from call_audit import audit_result_override

        for c in recent:
            cid = str(c.get("call_id") or "")
            if cid and audit_result_override(cid) == "caller_abandoned_before_intent":
                _abandoned_call_ids.add(cid)
    except Exception as exc:
        print(f"[WEEKLY_TRIAGE] abandoned override load failed: {exc!r}")
    ...
    for c in recent:
        ...
        is_transfer_type = call_type == "transfer"
        has_real_outcome = (
            is_message_type
            or is_cancellation_type
            or is_booking_type
            or is_transfer_type
            or has_booking
        )
        ...
        # Doctor marked this call as caller_abandoned (or migration override).
        if str(c.get("call_id") or "") in _abandoned_call_ids:
            c["outcome_class"] = "caller_abandoned_no_intent"
            caller_abandoned.append(
                {**c, "_triage_reasons": ["Ο καλών έκλεισε / χωρίς αίτημα (reviewed)"]}
            )
            continue

        # ── Doctor-reviewed caller-hangup policy ─────────────────────
        # 1) Caller closed before any intent: not an agent failure.
        # 2) Caller closed after a detected intent: unresolved/review, not an
        #    agent failure by itself.
        # 3) Clear user intent but no route/action recorded: agent failure.
        user_turns_count = int(c.get("user_turns") or 0)
        has_message = bool(str(c.get("message") or c.get("display_message") or "").strip())
        failure_point = str(c.get("failure_point") or "").strip().lower()
        flow_quality = str(c.get("flow_quality") or "").strip().lower()
        no_recorded_action_after_intent = (
            incomplete
            and not intent_completed
            and has_message
            and failure_point in {"agent_no_action_after_user_intent", "start_node", "start_only"}
        )
        if no_recorded_action_after_intent:
            reasons.append("Καθαρό αίτημα αλλά δεν καταγράφηκε route/tool/απάντηση από τον agent")
            c["outcome_class"] = "agent_no_action_after_user_intent"
            failed.append({**c, "_triage_reasons": reasons})
            continue

        caller_abandoned_no_intent = (
            incomplete and not intent_completed
            and user_turns_count <= 1
            and not has_message
            and intent in ("other", "unknown", "")
        )
        if caller_abandoned_no_intent:
            reasons.append("Ο καλών έκλεισε / χωρίς αίτημα")
            c["outcome_class"] = "caller_abandoned_no_intent"
            caller_abandoned.append({**c, "_triage_reasons": reasons})
            continue

        caller_left_after_detected_intent = (
            incomplete
            and not intent_completed
            and (
                failure_point in {
                    "caller_left_after_intent",
                    "caller_left_after_actionable_message",
                    "caller_left_partial_appointment",
                }
                or flow_quality == "caller_abandoned"
                or intent in {"appointment", "change", "message", "urgent", "cancellation", "confirmation"}
            )
        )
        if caller_left_after_detected_intent:
            reasons.append("Ο καλών έκλεισε αφού αναγνωρίστηκε intent — δεν μετρά ως αποτυχία agent")
            c["outcome_class"] = "caller_left_after_intent"
        ...
        if intent_completed:
            outcome_labels = {
                ...
                "transfer": "Ολοκληρώθηκε το intent: έγινε σύνδεση με τον γιατρό",
                "completed_message_update": "Ολοκληρώθηκε το intent: ενημερώθηκε το μήνυμα",
                "completed_urgent_message": "Ολοκληρώθηκε το intent: στάλθηκε επείγον μήνυμα",
                "transfer_connected": "Ολοκληρώθηκε το intent: έγινε σύνδεση με τον γιατρό",
                "transfer_attempted_not_failed": "Η σύνδεση με τον γιατρό ξεκίνησε χωρίς καταγεγραμμένη αποτυχία",
            }
            actual_outcome = c.get("outcome_class") or c.get("actual_outcome") or (
    ...
    try:
        ...
        scorecard = {
            ...
            "agent_failures": audit.get("agent_failures", 0),
            "agent_successes": audit.get("agent_successes", 0),
            "agent_controlled_success_rate_pct": audit.get(
                "agent_controlled_success_rate_pct"
            ),
            "manual_review_count": audit.get("manual_review_count", 0),
            "unresolved_count": audit.get("unresolved_count", 0),
            "task_completion_rate_pct": audit.get(
                "task_completion_rate_pct"
            ),
            "caller_ended_incomplete_count": audit.get(
                "caller_ended_incomplete_count", 0
            ),
            "tool_error_manual_review": audit.get(
                "tool_error_manual_review", 0
            ),
            "caller_abandoned_no_intent": audit.get(
                "caller_abandoned_no_intent", 0
            ),
            "caller_left_after_partial_appointment": audit.get(
                "caller_left_after_partial_appointment", 0
            ),
            "caller_left_after_actionable_message_needs_review": audit.get(
                "caller_left_after_actionable_message_needs_review", 0
            ),
            "caller_left_after_message_success": audit.get(
                "caller_left_after_message_success", 0
            ),
            "caller_left_after_message": audit.get(
                "caller_left_after_message", 0
            ),
            "caller_left_during_appointment": audit.get(
                "caller_left_during_appointment", 0
            ),
            "caller_left_after_success": audit.get("caller_left_after_success", 0),
            "caller_left_unresolved": audit.get("caller_left_unresolved", 0),
            "caller_left_after_intent": audit.get("caller_left_after_intent", 0),
            "caller_abandoned_before_intent": audit.get(
                "caller_abandoned_before_intent", 0
            ),
            "by_failure_stage": audit.get("by_failure_stage", {}),
        ...
        for group in (failed, needs_review, succeeded, caller_abandoned):
            for item in group:
    ...
    caller_abandoned_count = len(caller_abandoned)
    caller_left_non_agent_count = sum(
        1 for c in needs_review
        if c.get("outcome_class") in {"caller_left_after_intent", "caller_left_unresolved"}
    )
    actionable_total = total - caller_abandoned_count - caller_left_non_agent_count
    intent_completion_rate = round(100.0 * len(succeeded) / actionable_total, 1) if actionable_total else None
    ...
    try:
        ...
        audit_rows = load_call_audits(days=days)
        audit_counters = _dashboard_audit_bucket(
            audit_rows,
            days_divisor=max(1, days),
        )
    except Exception as exc:
        print(f"[WEEKLY_TRIAGE] counter audit unavailable: {exc!r}")
    ...
    return {
        ...
        "caller_abandoned_no_intent": caller_abandoned_count,
        "caller_left_non_agent": caller_left_non_agent_count,
        "success_rate_pct": intent_completion_rate,
        "failed": failed,
        "needs_review": needs_review,
        "succeeded": succeeded,
        "caller_abandoned": caller_abandoned,
        "caller_abandoned_count": caller_abandoned_count,
        "intent_completion_rate_pct": intent_completion_rate,
        "by_intent": by_intent,
        "latency": latency_summary,
        "scorecard": scorecard,
        "quality_gate": quality_gate,
        "summary": _safe_weekly_summary(
            recent,
            completed_calls=len(succeeded),
            audit_counters=audit_counters,
        ),
        "note": (
...
async def dashboard_weekly_triage(key: str = "", days: int = 7):
    try:
        return await _dashboard_weekly_triage_impl(key=key, days=days)
    except Exception as exc:
        import traceback

        print(f"[WEEKLY_TRIAGE] endpoint failed: {exc!r}")
        traceback.print_exc()
        return _safe_empty_weekly_result(days, error=str(exc))
...
<style>
  ...
  * { box-sizing:border-box; margin:0; padding:0; }
  ...
  .audit-card { background:var(--card); border:1px solid var(--rule); border-radius:10px; padding:14px 16px; }
  .audit-note { color:var(--ink-soft); font-size:.78rem; margin-top:8px; }
  .audit-table { width:100%; border-collapse:collapse; font-size:.82rem; margin-top:10px; }
  .audit-table th, .audit-table td { padding:6px 7px; border-bottom:1px solid var(--rule); text-align:left; vertical-align:top; }
  .audit-table th { color:var(--ink-soft); font-weight:700; font-size:.72rem; letter-spacing:.04em; text-transform:uppercase; }
  .audit-bars { display:grid; gap:7px; margin-top:12px; }
  .audit-bar-row { display:grid; grid-template-columns:130px 1fr 52px; gap:8px; align-items:center; font-size:.8rem; }
  .audit-track { height:9px; border-radius:999px; background:#eee6d8; overflow:hidden; }
  .audit-fill { height:100%; border-radius:999px; background:var(--accent); }
  .audit-intents { display:flex; flex-wrap:wrap; gap:6px; margin-top:10px; }
...
<div class="wrap">
  ...
  <div class="section" id="audit-comparison-section">
    <div class="section-head">
      <h2 style="cursor:pointer" onclick="toggleAuditPanel()">Σημερινό audit απόδοσης agent <span id="audit-caret" style="font-size:.7em">▾</span></h2>
      <span class="count" id="audit-generated">—</span>
    </div>
    <div class="audit-card" id="audit-comparison-card" style="display:none">Φόρτωση audit…</div>
    <div style="font-size:.72rem;color:var(--ink-soft);margin-top:4px">Οι κλήσεις όπου ο καλών έκλεισε ή δεν απάντησε δεν μετρούν ως αποτυχία agent.</div>
  </div>
...
function toggleAuditPanel(){
  const card = document.getElementById('audit-comparison-card');
  const caret = document.getElementById('audit-caret');
  if(!card) return;
  const hidden = card.style.display === 'none';
  card.style.display = hidden ? 'block' : 'none';
  if(caret) caret.textContent = hidden ? '▴' : '▾';
  if(hidden && card.innerHTML === 'Φόρτωση audit…') fetchAuditComparison();
}
...
let _lastAuditComparisonFetchAt = 0;
...
async function fetchCalls(){
  ...
  try{
    ...
    if(_due(_lastAuditComparisonFetchAt, DASHBOARD_INSIGHTS_REFRESH_MS)){
      _lastAuditComparisonFetchAt = Date.now();
...
function _auditPct(v){
  return (v===null || v===undefined || v==='') ? '—' : `${v}%`;
}
function _auditMetricRow(label, key, data, asPct){
  const today = (data.today||{})[key];
  const yesterday = (data.yesterday||{})[key];
  const last7 = (data.last_7_days_avg||{})[key];
  const prev = (data.previous_week_avg||{})[key];
  const fmt = asPct ? _auditPct : _auditNum;
  return `<tr><td>${label}</td><td>${fmt(today)}</td><td>${fmt(yesterday)}</td><td>${fmt(last7)}</td><td>${fmt(prev)}</td></tr>`;
}
function _auditBar(label, value, maxValue){
  const v = Number(value||0);
  const max = Math.max(1, Number(maxValue||0));
  const pct = Math.max(0, Math.min(100, Math.round(100*v/max)));
  return `<div class="audit-bar-row"><span>${label}</span><div class="audit-track"><div class="audit-fill" style="width:${pct}%"></div></div><span>${_auditNum(value)}</span></div>`;
}
async function fetchAuditComparison(){
  const card = document.getElementById('audit-comparison-card');
  if(card && card.style.display === 'none' && card.innerHTML !== 'Φόρτωση audit…') return;
  try{
    const r = await fetch('/dashboard/audit-comparison'+(KEY?'?key='+KEY:''));
    const el = document.getElementById('audit-comparison-card');
    if(!el) return;
    if(!r.ok){ el.innerHTML = 'Δεν ήταν διαθέσιμο το σημερινό audit.'; return; }
    const d = await r.json();
    const gen = document.getElementById('audit-generated');
    if(gen && d.generated_at){
      try{ gen.textContent = new Date(d.generated_at).toLocaleString('el-GR',{day:'2-digit',month:'2-digit',hour:'2-digit',minute:'2-digit',timeZone:'Asia/Nicosia'}); }
      catch(e){ gen.textContent = d.generated_at; }
    }
    const today = d.today || {};
    const last7 = d.last_7_days_avg || {};
    const maxBar = Math.max(
      Number(today.completed_successfully||0), Number(last7.completed_successfully_avg_per_day||0),
      Number(today.unresolved_count||0), Number(last7.unresolved_count_avg_per_day||0),
      Number(today.caller_ended_incomplete_count||0),
      Number(today.tool_errors||0), Number(last7.tool_errors_avg_per_day||0),
      Number(today.urgent_count||0), Number(last7.urgent_count_avg_per_day||0),
      Number(today.transfer_connected||0), Number(last7.transfer_connected_avg_per_day||0),
      Number(today.agent_failure_count||0), Number(last7.agent_failure_count_avg_per_day||0),
      Number(today.agent_success_count||0), Number(last7.agent_success_count_avg_per_day||0),
      Number(today.manual_review_count||0), Number(last7.manual_review_count_avg_per_day||0),
      1
    );
    const intents = d.intent_distribution_today || {};
    const intentTags = Object.entries(intents).length
      ? Object.entries(intents).sort((a,b)=>b[1]-a[1]).map(([k,v])=>`<span class="tag">${k}: ${v}</span>`).join('')
      : '<span class="tag">χωρίς σημερινά labels</span>';
    const failureStages = today.failure_stage_distribution || {};
    const failureStageTags = Object.entries(failureStages).length
      ? Object.entries(failureStages).map(([k,v])=>`<span class="tag">${k}: ${v}</span>`).join('')
      : '<span class="tag">καμία αποτυχία agent</span>';
    const sample = d.sample_sizes || {};
    const smallSample = Number(sample.today||0) === 0
      ? 'Δεν υπάρχουν ακόμη αρκετές σημερινές κλήσεις.'
      : (Number(sample.today||0) < 5 ? 'Μικρό δείγμα σήμερα — δείτε το μαζί με τον εβδομαδιαίο μέσο όρο.' : '');
    const agentFailureMessage = Number(today.agent_failure_count||0) === 0
      ? 'Καμία καταγεγραμμένη αποτυχία agent σήμερα.'
      : `${_auditNum(today.agent_failure_count)} καταγεγραμμένες αποτυχίες agent σήμερα.`;
    el.innerHTML = `
      <div><b>${_auditNum(today.total_calls)}</b> κλήσεις σήμερα · <b>${_auditNum(today.completed_successfully)}</b> ολοκληρωμένες · <b>${_auditNum(today.agent_failure_count)}</b> αποτυχίες agent · <b>${_auditNum(today.caller_ended_incomplete_count)}</b> caller-ended/incomplete</div>
      <div class="audit-note"><b>Agent-controlled success rate: ${_auditPct(today.agent_controlled_success_rate_pct)}</b> (${_auditNum(today.agent_success_count)} επιτυχίες / ${_auditNum(today.agent_failure_count)} αποτυχίες). ${agentFailureMessage}</div>
      <div class="audit-note">Το completion rate περιλαμβάνει κλήσεις που έκλεισε ο καλών. Οι αποτυχίες agent μετρώνται ξεχωριστά.</div>
      ${smallSample ? `<div class="audit-note">${smallSample}</div>` : ''}
      <table class="audit-table">
        <thead><tr><th>Metric</th><th>Σήμερα</th><th>Χθες</th><th>7-day avg</th><th>Προηγ. εβδομ.</th></tr></thead>
        <tbody>
          ${_auditMetricRow('Κλήσεις', 'total_calls', d, false)}
          ${_auditMetricRow('Task completion rate', 'task_completion_rate_pct', d, true)}
          ${_auditMetricRow('Agent-controlled success', 'agent_controlled_success_rate_pct', d, true)}
          ${_auditMetricRow('Επιτυχίες agent', 'agent_success_count', d, false)}
          ${_auditMetricRow('Αποτυχίες agent', 'agent_failure_count', d, false)}
          ${_auditMetricRow('Tool/manual review', 'tool_error_manual_review', d, false)}
          ${_auditMetricRow('Σύνολο για έλεγχο', 'manual_review_count', d, false)}
          ${_auditMetricRow('Caller έκλεισε μετά από επιτυχία', 'caller_left_after_success', d, false)}
          ${_auditMetricRow('Caller abandoned — χωρίς intent', 'caller_abandoned_no_intent', d, false)}
          ${_auditMetricRow('Caller έφυγε σε μερικό ραντεβού', 'caller_left_after_partial_appointment', d, false)}
          ${_auditMetricRow('Actionable μήνυμα — χρειάζεται έλεγχο', 'caller_left_after_actionable_message_needs_review', d, false)}
          ${_auditMetricRow('Caller έκλεισε μετά από επιτυχές μήνυμα', 'caller_left_after_message_success', d, false)}
          ${_auditMetricRow('Caller-ended/incomplete', 'caller_ended_incomplete_count', d, false)}
          ${_auditMetricRow('Unresolved — μόνο agent/review', 'unresolved_count', d, false)}
          ${_auditMetricRow('Μηνύματα', 'message_success', d, false)}
          ${_auditMetricRow('Ραντεβού', 'booking_success', d, false)}
          ${_auditMetricRow('Ακυρώσεις', 'cancellation_success', d, false)}
          ${_auditMetricRow('Transfer connected', 'transfer_connected', d, false)}
          ${_auditMetricRow('Επείγοντα', 'urgent_count', d, false)}
          ${_auditMetricRow('Tool errors', 'tool_errors', d, false)}
          ${_auditMetricRow('Stuck/silence', 'stuck_or_silence_count', d, false)}
          ${_auditMetricRow('P95 latency', 'p95_latency_ms', d, false)}
        </tbody>
      </table>
      <div class="audit-bars">
        ${_auditBar('Agent success σήμερα', today.agent_success_count, maxBar)}
        ${_auditBar('Αποτυχίες agent σήμερα', today.agent_failure_count, maxBar)}
        ${_auditBar('Manual review σήμερα', today.manual_review_count, maxBar)}
        ${_auditBar('Caller-ended/incomplete σήμερα', today.caller_ended_incomplete_count, maxBar)}
        ${_auditBar('Caller abandoned / χωρίς αίτημα σήμερα', today.caller_abandoned_no_intent, maxBar)}
        ${_auditBar('Transfer σήμερα', today.transfer_connected, maxBar)}
        ${_auditBar('Μηνύματα σήμερα', today.message_success, maxBar)}
        ${_auditBar('Ραντεβού σήμερα', today.booking_success, maxBar)}
      </div>
      <div class="audit-note" style="opacity:.7">Calls ended/disconnected σήμερα: ${_auditNum(today.caller_hangup_total)} (κάθε κλήση κάποια στιγμή αποσυνδέεται — δεν είναι δείκτης αποτυχίας).</div>
      <div class="audit-note"><b>Σημεία αποτυχίας agent</b> ${failureStageTags}</div>
      <div class="audit-note"><b>Intent distribution</b> — δεν είναι human-labeled accuracy. Τα labels προέρχονται από dashboard/agent audit.</div>
      <div class="audit-intents">${intentTags}</div>
    `;
  }catch(e){
    const el = document.getElementById('audit-comparison-card');
    if(el) el.innerHTML = 'Δεν ήταν διαθέσιμο το σημερινό audit.';
  }
}
...
async function runWeeklyTriage(){
  ...
  if(!panel) return;
  if(btn){ btn.textContent = '⏳ Έλεγχος...'; btn.disabled = true; }
  try{
    const r = await fetch('/dashboard/weekly_triage'+(KEY?'?key='+KEY:''));
    const contentType = r.headers.get('content-type') || '';
    if(!contentType.includes('application/json')){
      throw new Error(`weekly_triage returned ${r.status} ${contentType}`);
    }
    const d = await r.json();
    if(!r.ok){ panel.innerHTML = `<div class="empty">Σφάλμα: ${d.error||'άγνωστο'}</div>`; return; }

    const sevColor = {failed:'#ef4444', needs_review:'#f59e0b', succeeded:'#22c55e', caller_abandoned:'#94a3b8'};
    const sevLabel = {failed:'🔴 Αποτυχίες agent', needs_review:'🟡 Χρειάζονται έλεγχο', succeeded:'🟢 Πέτυχαν', caller_abandoned:'⚪ Caller abandoned / χωρίς αίτημα'};
    const intentOptions = ['appointment','message','confirmation','change','cancellation','urgent','other','caller_abandoned'];
    const intentOptionLabel = (i) => i === 'caller_abandoned' ? 'Ο καλών έκλεισε / χωρίς αίτημα' : i;
    ...
    let html = `<div class="section"><div class="section-head"><h2>Έλεγχος ${d.period_days} ημερών</h2>
      <span class="count">${d.total_calls} κλήσεις · ${d.succeeded_count||0} ολοκληρωμένες · ${(d.scorecard||{}).agent_failures||0} αποτυχίες agent · ${(d.scorecard||{}).caller_ended_incomplete_count||0} caller-ended/incomplete</span></div>`;
    html += `<div class="stats" style="margin:8px 0 10px">
      ${metric('Σύνολο', ws.total_calls)}
      ${metric('Ολοκληρωμένες', ws.completed_calls)}
      ${metric('Χρειάζονται έλεγχο', (d.scorecard||{}).unresolved_count)}
      ${metric('Caller abandoned', d.caller_abandoned_count)}
      ${metric('Μηνύματα', ws.messages)}
      ${metric('Ραντεβού', ws.appointments)}
      ${metric('Επείγοντα μηνύματα', ws.urgent_messages)}
      ${metric('Transfers', ws.transfers)}
      ${metric('Stuck/silence', ws.stuck_or_silence)}
      ${metric('Tool errors', ws.tool_errors)}
    </div>`;
    const sc = d.scorecard || {};
    html += `<div class="stats" style="margin:8px 0 10px">
      ${metric('Agent-controlled success', sc.agent_controlled_success_rate_pct, '%')}
      ${metric('Task completion rate', sc.task_completion_rate_pct, '%')}
      ...
      ${metric('Αποτυχίες agent', sc.agent_failures)}
      ${metric('Manual review', sc.manual_review_count)}
      ${metric('Tool errors/review', sc.tool_error_manual_review)}
      ${metric('Caller abandoned — χωρίς intent', sc.caller_abandoned_no_intent)}
      ${metric('Caller έφυγε σε μερικό ραντεβού', sc.caller_left_after_partial_appointment)}
      ${metric('Actionable μήνυμα — έλεγχος', sc.caller_left_after_actionable_message_needs_review)}
      ${metric('Caller έφυγε μετά επιτυχές μήνυμα', sc.caller_left_after_message_success)}
      ${metric('Caller-ended/incomplete', sc.caller_ended_incomplete_count)}
    </div>`;
    if((sc.agent_failures||0)===0){
      html += `<div style="font-size:.82rem;color:#166534;margin-bottom:8px">Καμία καταγεγραμμένη αποτυχία agent σήμερα.</div>`;
    }
    html += `<div style="font-size:.75rem;color:var(--ink-soft);margin-bottom:8px">
      Το completion rate περιλαμβάνει κλήσεις που έκλεισε ο καλών. Οι αποτυχίες agent μετρώνται ξεχωριστά.
    </div>`;
    ...
    const failureStages = sc.by_failure_stage || {};
    if(Object.keys(failureStages).length){
      html += `<div style="font-size:.78rem;color:var(--ink-soft);margin-bottom:8px"><b>Σημεία αποτυχίας agent:</b> `+
        Object.entries(failureStages).map(([name,count]) => `${name}: ${count}`).join(' · ')+
        `</div>`;
    }
    ...
    for(const group of ['failed','needs_review','succeeded','caller_abandoned']){
      const items = d[group] || [];
      ...
      html += items.slice(0,20).map(c => {
        ...
        const labelControl = c.call_id ? `<div style="margin-top:6px;display:flex;align-items:center;gap:6px">
          ...
          <select onchange="labelCallIntent('${c.call_id}',this)" style="font-size:.72rem;padding:3px 5px;border-radius:6px">
            ...
            ${intentOptions.map(i=>`<option value="${i}" ${c._expected_intent===i?'selected':''}>${intentOptionLabel(i)}</option>`).join('')}
          </select>
    ...
    console.error('weekly_triage load failed', e);
    panel.innerHTML = `<div class="empty">Σφάλμα φόρτωσης ελέγχου: ${String(e.message||e)}</div>`;
  }finally{
    if(btn){ btn.textContent = '📋 Έλεγχος εβδομάδας'; btn.disabled = false; }
  }
}
