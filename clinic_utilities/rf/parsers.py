...
def parse_medications(text: str) -> dict:
    ...
    nsaids = ranked(grouped["nsaid"])
    others = ranked(grouped["other"])
    return {
        "nsaid_candidates": [x.to_dict() for x in nsaids],
...
def parse_physio_dates(text: str) -> dict:
    ...
    values = sorted(found)
    return {
        "dates": [v.isoformat() for v in values],
        "start_date": values[0].isoformat() if values else "",
        "end_date": values[-1].isoformat() if values else "",
        "treatment_count": len(values),
        "invalid_or_ambiguous_tokens": list(dict.fromkeys(invalid)),
    }
