...
class CU1GreekReferralFormatter(_BaseGreekFormatter):
    ...
    def _format_lateral_elbow_short(self, draft: Mapping[str, Any]) -> str:
        ...
        if findings:
            sentences.append(f"Κλινικά ευρήματα: {self._join_greek(findings)}.")
        if function:
            sentences.append(f"Λειτουργικός περιορισμός: {self._join_greek(function)}.")
        if work_context:
            sentences.append(f"Καταγεγραμμένο επαγγελματικό/αθλητικό πλαίσιο: {work_context}.")
    ...
    def _format_lateral_elbow_detailed(self, draft: Mapping[str, Any]) -> str:
        ...
        sections.append(
            ...
            "• Τεχνικές κινητοποίησης, άκαμπτη περίδεση, ξηρά βελόνηση ή άλλα επιλεγμένα συμπληρωματικά μέσα μπορούν να χρησιμοποιηθούν όταν είναι κατάλληλα, αλλά δεν αποτελούν το κύριο περιεχόμενο του σταδίου.\n\n"
            "Δείκτες προόδου:\n"
        ...
        monitoring = (
            ...
            "PSFS ή άλλο εργαλείο ειδικό για τη δραστηριότητα όταν υπάρχει απαιτητική λειτουργία, καθώς και δύναμη λαβής "
            "χωρίς πόνο/μέγιστη δύναμη λαβής και εύρος κίνησης όταν είναι σχετικά με το κλινικό έλλειμμα. Οι μετρήσεις δεν "
            "αποτελούν από μόνες τους καθολικά κριτήρια μετάβασης ή εξόδου."
        )
        if measurements:
        ...
        adjunct_text = (
            ...
            "ενώ ο θεραπευτικός υπέρηχος δεν πρέπει να αντιμετωπίζεται ως επαρκής αυτόνομη θεραπεία. "
