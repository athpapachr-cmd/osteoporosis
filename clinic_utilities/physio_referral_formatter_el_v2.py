...
class CU1GreekReferralFormatter(_BaseGreekFormatter):
    ...
    def _format_lateral_elbow_short(self, draft: Mapping[str, Any]) -> str:
        ...
        parts.append(
            "Τέλος λειτουργική/επαγγελματική επανένταξη και μείωση κινδύνου υποτροπής με διαχείριση φορτίου, "
            "εργονομικές/τεχνικές προσαρμογές και πρόγραμμα διατήρησης δύναμης/αντοχής. Παθητικά μέσα μόνο "
            "επικουρικά· όχι αντί ενεργητικής αποκατάστασης. Πρόοδος βάσει κλινικής/λειτουργικής ανταπόκρισης, "
            "χωρίς καθολικά αριθμητικά κριτήρια."
        )
    ...
    def _format_lateral_elbow_detailed(self, draft: Mapping[str, Any]) -> str:
        ...
        sections = [
            ...
            (
                "ΣΤΑΔΙΟ 3 — ΛΕΙΤΟΥΡΓΙΚΗ ΕΠΑΝΕΝΤΑΞΗ / ΠΡΟΛΗΨΗ ΥΠΟΤΡΟΠΗΣ\n"
                "Στόχοι: επάνοδος στις καθημερινές, επαγγελματικές/αθλητικές απαιτήσεις, αυτοδιαχείριση και μείωση κινδύνου υποτροπής.\n"
                "Κατευθύνσεις: προοδευτική έκθεση στις πραγματικές απαιτήσεις, λειτουργική χρήση/λαβή, διαχείριση "
                "φορτίου, εργονομικές/τεχνικές προσαρμογές όπου χρειάζονται και πρόγραμμα διατήρησης δύναμης/αντοχής.\n"
                "Πρόοδος/ολοκλήρωση: ουσιαστική λειτουργική βελτίωση, ανοχή στο συνήθες φορτίο και ικανότητα αυτοδιαχείρισης, "
                "χωρίς καθολικά αριθμητικά κριτήρια."
            ),
            (
