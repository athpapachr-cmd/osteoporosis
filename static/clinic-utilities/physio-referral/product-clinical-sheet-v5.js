'use strict';
// Compact clinical-picture interaction v5. Presentation only: clinical IDs,
// evidence, safety and projection authority remain unchanged. Optional symptom
// detail no longer blocks the first parent selection.
const V4_CLINICAL = Object.freeze({
  pain:{label:'Πόνος',prompt:'Πού εντοπίζεται;'},
  stiffness:{label:'Δυσκαμψία',prompt:'Πότε εμφανίζεται;'},
  weakness:{label:'Αδυναμία',prompt:'Τι έχει διαπιστωθεί στην εξέταση;'},
...
function v4RenderGrid() {
  ...
  $$('[data-clinical-v4]').forEach(b=>{
    ...
    if(badge){
      const optionalDetail=active && kind!=='function' && count===0;
      badge.hidden=!(count || optionalDetail);
      badge.textContent=count?`· ${count}`:(optionalDetail?'· λεπτομέρειες':'');
    }
    const detailHint=active && kind!=='function' && count===0?' · δεύτερο πάτημα για προαιρετικές λεπτομέρειες':'';
    b.setAttribute('aria-label',V4_CLINICAL[kind].label+(count?` · ${count} ενεργοί προσδιορισμοί`:detailHint));
  });
}
...
function v4SheetNodes(kind) {
  ...
  if(kind==='pain') {
    ...
    nodes.push(make('div',{class:'clinical-options-v4'},[
      ...
      v4Option('Αδυναμία τετρακεφάλου',{'data-q-weakness':'quadriceps_exam'}),
      v4Option('Εμφανής ατροφία τετρακεφάλου',{'data-q-atrophy-quadriceps-v5':''}),
    ]));
  } else {
    nodes.push(make('div',{class:'clinical-options-v4'},[
...
function v4SyncClinicalSheet() {
  ...
  $$(`#sheetBody [data-q-atrophy-quadriceps-v5]`).forEach(b=>b.setAttribute('aria-pressed',String(qualifierState.visible_atrophy && qualifierState.atrophy_location==='quadriceps')));
...
function v5HandleClinicalTap(kind,source) {
  if(kind==='function'){v4OpenClinicalSheet(kind,source);return;}
  if(!v4Active(kind)){
    if(v4EnsureParent(kind)) changed('clinical_picture_'+kind+'_selected'); else paintStatus();
    return;
  }
  v4OpenClinicalSheet(kind,source);
}
...
document.addEventListener('click',event=>{
  ...
  if(b.dataset.clinicalV4){v5HandleClinicalTap(b.dataset.clinicalV4,b);return;}
  ...
  if(b.hasAttribute('data-q-atrophy-quadriceps-v5')){
    const selected=qualifierState.visible_atrophy && qualifierState.atrophy_location==='quadriceps';
    qualifierState.visible_atrophy=!selected;
    qualifierState.atrophy_location=selected?null:'quadriceps';
    changed('atrophy_location');
