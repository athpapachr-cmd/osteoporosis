...
class KneeOACockpitBrowserTests(unittest.TestCase):
    ...
    def open_safety(self):
        if self.page.locator("#advancedToggle").get_attribute("aria-expanded") != "true": self.page.locator("#advancedToggle").click()
        self.page.locator("#advanced .v3-category-row[data-v3-category=safety]").click(); expect(self.page.locator("#sheet")).to_be_visible()
    ...
    def test_atypical_review_and_explicit_safety_gate_on_protected_transport(self):
        self.ready();before=self.page.locator("#referralText").inner_text();self.open_safety()
        details=self.page.locator("#sheet [data-v51-atypical-disclosure]");details.locator("summary").click()
        self.page.locator('#sheet [data-v51-atypical="recent_trauma"]').click();self.page.locator('#sheet [data-v51-atypical="hot_swollen_joint"]').click()
        expect(self.page.locator("#copy")).to_be_enabled();expect(self.page.locator('#sheet [data-select="infection_or_septic_joint_concern"]')).to_have_attribute("aria-pressed","false")
